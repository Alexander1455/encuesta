...
const MuestraEncuesta = () => {
  ...
  useEffect(() => {
    const obtenerEncuestas = async () => {
      try {
        const response = await axios.get('http://localhost:5173/encuestas');
      ...
      <div className="py-8">
        {encuestas.map((encuesta, index) => (
          ...
          <div key={index} className="flex flex-col bg-white shadow-md px-4 sm:px-6 md:px-8 lg:px-10 py-8 rounded-md w-full max-w-md">
            <div className="font-bold text-xl mb-2">{encuesta.titulo}</div>
            <div className="">
              <div className="border-b-4 border-neutral-950 py-2">
                <h2>Descripción</h2>
              </div>
              <p className="">{encuesta.descripcion}</p>
